...
export class DashboardAdminComponent extends BaseComponent {
  ...
  renderData() {
    ...
    setTimeout(() => {
      ...
      const chartEstado = this._renderChartEstado(data);
      const chartInstitucion = this._renderChartInstitucion(data);
      const chartTendencia = this._renderChartTendencia(data);

      window.addEventListener('resize', () => {
        if (chartEstado) chartEstado.resize();
        if (chartInstitucion) chartInstitucion.resize();
  ...
  _renderChartEstado(data) {
    const el = this.querySelector('#chartEstado');
    if (!el) return null;
    ...
    const chartEstado = window.echarts.init(el);
  ...
  _renderChartInstitucion(data) {
    const el = this.querySelector('#chartInstitucion');
    if (!el) return null;

    const chartInstitucion = window.echarts.init(el);
    ...
    return chartInstitucion;
  }

  _renderChartTendencia(data) {
    const el = this.querySelector('#chartTendencia');
    if (!el) return null;

    const chartTendencia = window.echarts.init(el);
