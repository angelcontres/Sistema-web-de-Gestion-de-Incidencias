  ...
  renderData() {
    ...
    setTimeout(() => {
        ...
        const estadoData = (data.distribucion_estado || []).map(item => ({
            name: String(item.metric), value: Number(item.value), itemStyle: { color: estadoColors[item.metric] || '#cbd5e1' }
        }));
        chartEstado.setOption({
            tooltip: { trigger: "item", formatter: "{b}: {c} ({d}%)" },
            legend: { orient: "horizontal", bottom: "0%", textStyle: { color: "#64748b" } },
            series: [{
                name: "Estado", type: "pie", radius: ["40%", "70%"],
                avoidLabelOverlap: false,
                itemStyle: { borderRadius: 8, borderColor: "#fff", borderWidth: 2 },
                label: { show: estadoData.length > 0, formatter: "{b}: {c}", color: "#64748b" },
                data: estadoData.length ? estadoData : [{ name: "Sin datos", value: 0, itemStyle: { color: "#e2e8f0" } }]
            }]
        });
        ...
        chartInstitucion.setOption({
            ...
            xAxis: { type: "category", data: instxAxis, axisLabel: { color: "#64748b", rotate: 15 }, axisLine: { lineStyle: { color: "#e2e8f0" } } },
        ...
        chartTendencia.setOption({
            ...
            xAxis: { type: "category", data: xAxisData, axisLabel: { color: "#64748b" }, axisLine: { lineStyle: { color: "#e2e8f0" } } },
            yAxis: { type: "value", name: "Reportes", nameTextStyle: { color: "#64748b" }, axisLabel: { color: "#64748b" }, splitLine: { lineStyle: { type: "dashed", color: "#e2e8f0" } } },
            series: [{
                name: "Reportes", type: "line", smooth: true, symbolSize: 8,
