...
export class IncidentFormStateHelper {
  ...
  async cargarDatosEdicion(id) {
    ...
    try {
      ...
      this._populateBasicData(inc);
      await this._populateLocationData(inc.direccion);
      this.component.locationManager.actualizarIndicadorMinimalista();
      this._populateMedia(inc);
      this._applyRoleBasedUI(inc);
    } catch (e) {
      console.error(e);
  ...
  async _populateLocationData(dir) {
    if (!dir) return;
    ...
    await this._loadTerritoryDropdowns(dir.territorio);
  }

  async _loadTerritoryDropdowns(terr) {
    if (!terr) return;
    ...
    const { n1, n2, n3 } = this._getTerritoryLevels(terr);
  ...
  _getTerritoryLevels(terr) {
    if (terr.parent && terr.parent.parent) {
      return { n1: terr.parent.parent.id, n2: terr.parent.id, n3: terr.id };
    }
    if (terr.parent) {
      return { n1: terr.parent.id, n2: terr.id, n3: null };
    }
    return { n1: terr.id, n2: null, n3: null };
  }
