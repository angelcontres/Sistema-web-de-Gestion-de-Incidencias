...
describe('HistorialIndexComponent', () => {
  ...
  it('onInit - debería configurar la tabla y cargar datos', async () => {
    ...
    // Test columns render and format
    const idColumn = columns.find(c => c.header === 'ID');
    expect(idColumn.format(5)).toBe('#5');

    const clasificacionColumn = columns.find(c => c.header === 'Clasificación');
    expect(clasificacionColumn.render({ tipo: { nombre: 'Tipo1' }, subTipo: { nombre: 'SubTipo1' } })).toContain('Tipo1');
    expect(clasificacionColumn.render({ tipo: { nombre: 'Tipo2' }, sub_tipo: { nombre: 'SubTipo2' } })).toContain('SubTipo2');
    ...
    expect(ubiColumn.render({ direccion: { detalle: 'Dir', territorio: { pais: { nombre: 'Peru' } } } })).toContain('Peru');
    expect(ubiColumn.render({})).toContain('Sin dirección.');

    const prioColumn = columns.find(c => c.header === 'Prioridad');
    expect(prioColumn.render({ prioridad: { nombre: 'Alta', color_hex: '#ff0000' } })).toContain('Alta');
