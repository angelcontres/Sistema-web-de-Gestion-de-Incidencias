...
class DashboardMetricsQuery
{
    ...
    public function getMetrics($role, $user)
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(15), function () use ($role, $user) {
            ...
            if ($role === 'Ciudadano') {
                $data = $this->getCiudadanoMetrics($user, $startOfRange);
            } elseif ($role === 'Institucion') {
                $data = $this->getInstitucionMetrics($user, $startOfRange);
            } elseif ($role === 'Supervisor' || $role === 'Admin') {
                $data = $this->getSupervisorMetrics($startOfRange);
            }
    ...
    private function getCiudadanoMetrics($user, $startOfRange): array
    {
        $data = [];
        $data['kpis'] = [
    ...
    private function getInstitucionMetrics($user, $startOfRange): array
    {
    ...
    private function getSupervisorMetrics($startOfRange): array
    {
        $data = [];
        $data['kpis'] = [
    ...
    {
        // Aseguramos que cada elemento sea un array puro para evitar problemas de serialización en el caché
